...
// --- NEW GITHUB PAGES FETCH API WRAPPER ---
async function apiCall(action, ...params) {
    const response = await fetch(API_URL, {
        method: 'POST',
        // Send as plain text to avoid CORS Preflight Options blocks
...
function loadApp() {
  initDB(() => {
    loadLocalCache((hasCache) => {
      ...
      // Use standard fetch instead of google.script.run
      apiCall('getInitialData')
        ...
        .catch(err => {
            console.error(err);
            updateSyncStatus('Offline'); document.getElementById('loaderOverlay').classList.add('d-none');
        });
    });
  });
}
...
function saveToQueue(action, params) {
  if(!db) return;
  try { let tx = db.transaction('syncQueue', 'readwrite'); tx.objectStore('syncQueue').add({ id: Date.now().toString(), action: action, params: params }); updateSyncStatus('Pending'); } catch(e){}
}

function processQueue() {
  if (!navigator.onLine || !db) return;
  try {
    let tx = db.transaction('syncQueue', 'readonly'); let req = tx.objectStore('syncQueue').getAll();
    req.onsuccess = () => {
      let queue = req.result; if (queue.length === 0) return;
      updateSyncStatus('Syncing...'); let item = queue[0];
      
      apiCall(item.action, ...item.params)
        .then(() => {
            let dTx = db.transaction('syncQueue', 'readwrite'); dTx.objectStore('syncQueue').delete(item.id); dTx.oncomplete = () => processQueue();
        })
...
function executeSave(action, ...params) {
  ...
  apiCall(action, ...params)
    .then(() => { loadApp(); })
    .catch(() => { saveToQueue(action, params); });
}

function confirmDelete(type, id) {
    if(confirm(`Delete this ${type}? Linked records will also be removed.`)) {
...
function optimisticUpdate(action, params) {
  ...
  if(action === 'addClient') loadedData.clients.push({ ID: tempId, Name: params[0], 'Contact Name': params[1], Email: params[2], Mobile: params[3] });
    ...
    loadedData.invoices.push({ 'Invoice ID': tempId, 'Client ID': params[0].clientId, 'Client Name': params[0].clientName, Date: params[0].date, 'Invoice Number': params[0].invNum, 'Net Amount': params[0].netAmount });
  }
  else if (action === 'deleteRecord') {
    let type = params[0]; let id = params[1];
    if(type === 'client') {
        loadedData.clients = loadedData.clients.filter(c => c.ID !== id);
        let invIds = loadedData.invoices.filter(i => i['Client ID'] === id).map(i => i['Invoice ID']);
        loadedData.invoices = loadedData.invoices.filter(i => i['Client ID'] !== id);
        loadedData.receipts = loadedData.receipts.filter(r => !invIds.includes(r['Invoice ID']) && r['Client ID'] !== id);
    } else if(type === 'invoice') {
        loadedData.invoices = loadedData.invoices.filter(i => i['Invoice ID'] !== id);
        loadedData.receipts = loadedData.receipts.filter(r => r['Invoice ID'] !== id);
...
function calculateBalances() {
  if(!loadedData.invoices) loadedData.invoices = [];
  if(!loadedData.receipts) loadedData.receipts = [];
  if(!loadedData.clients) loadedData.clients = [];
  ...
  loadedData.invoices.forEach(inv => {
    ...
    inv.paidAmount = paid;
    inv.balance = (parseFloat(inv['Net Amount']) || 0) - paid;
  });
...
function populateDropdowns() {
  ...
  if(rList) { rList.innerHTML = ''; loadedData.clients.forEach(c => rList.innerHTML += `<option value="${c.Name}">`); }
}

let currentZoom = 1;
function zoomInvoice(delta) {
    currentZoom += delta;
    if(currentZoom < 0.3) currentZoom = 0.3;
    if(currentZoom > 2.5) currentZoom = 2.5;
    const scaleWrapper = document.getElementById('invoicePreviewScaleWrapper');
...
function setInitialZoom() {
    const bodyWrapper = document.getElementById('invoicePreviewBodyWrapper');
    const bodyWidth = bodyWrapper.clientWidth - 30;
...
function openInvoicePreview(invId) {
  ...
  let items = loadedData.invoiceItems.filter(i => i['Invoice ID'] === invId);
  let itemsHtml = items.map(i => `<tr><td><strong class="text-dark">${i.Name}</strong><br><span class="text-muted">${i.Description}</span></td><td>${parseFloat(i.Rate).toFixed(2)}</td><td>${i.Quantity}</td><td class="text-end fw-bold">${parseFloat(i.Amount).toFixed(2)}</td></tr>`).join('');
  let contactHtml = client ? `${client['Contact Name'] || ''}<br>${client.Mobile || ''}<br>${client.Email || ''}` : '';
  let advanceRow = '';
  if (bal < 0) { bal = 0; advanceRow = `<tr class="bg-light text-success"><td><h6 class="mt-2 fw-bold mb-2">Overpaid / Advance</h6></td><td><h6 class="mt-2 fw-bold mb-2">${Math.abs(inv.balance).toFixed(2)}</h6></td></tr>`; }

  let invoiceHTML = `<div class="row mb-4 pb-3 border-bottom"><div class="col-7">${logoHtml}<h5 class="fw-bold mb-1 text-dark">${loadedData.company['Company Name'] || ''}</h5><p class="text-muted mb-0" style="white-space: pre-line; line-height: 1.4;">${loadedData.company['Address'] || ''}\n${loadedData.company['Contact'] || ''}</p></div><div class="col-5 text-end"><h2 class="fw-bold" style="color: #4f46e5; letter-spacing: -0.5px;">INVOICE</h2><p class="mb-1"><span class="text-muted fw-bold me-1">Invoice #:</span> <span class="fw-bold">${inv['Invoice Number']}</span></p><p class="mb-0"><span class="text-muted fw-bold me-1">Date:</span> <span>${inv.Date}</span></p></div></div><div class="row mb-3"><div class="col-12"><p class="text-muted fw-bold mb-1" style="letter-spacing: 0.5px;">BILLED TO</p><h6 class="fw-bold text-dark">${inv['Client Name']}</h6><div class="text-muted" style="line-height: 1.4;">${contactHtml}</div></div></div><table class="table table-sm table-bordered border-dark"><thead style="background-color: #f8fafc;"><tr><th>Description</th><th style="width: 15%;">Rate</th><th style="width: 10%;">Qty</th><th class="text-end" style="width: 20%;">Amount</th></tr></thead><tbody>${itemsHtml}</tbody></table><div class="row"><div class="col-6"></div><div class="col-6"><table class="table table-sm table-borderless text-end mb-0"><tr><td class="text-muted">Subtotal</td><td>${parseFloat(inv['Total Amount']||0).toFixed(2)}</td></tr><tr><td class="text-muted">Discount</td><td>${parseFloat(inv['Discount']||0).toFixed(2)}</td></tr><tr class="border-bottom border-dark"><td class="text-muted">Round Off</td><td>${parseFloat(inv['Round Off']||0).toFixed(2)}</td></tr><tr><td class="fw-bold text-dark pt-2">Net Total</td><td class="fw-bold text-dark pt-2">${net.toFixed(2)}</td></tr><tr><td class="text-muted pb-2">Amount Paid</td><td class="text-success pb-2">${paid.toFixed(2)}</td></tr><tr style="background-color: #f8fafc;"><td><h6 class="mt-2 fw-bold text-dark mb-2">Balance Due</h6></td><td><h6 class="mt-2 fw-bold text-dark mb-2">${bal.toFixed(2)}</h6></td></tr>${advanceRow}</table></div></div><div class="row print-footer align-items-end mt-4 pt-4"><div class="col-7"><p class="text-dark fw-bold mb-1">Terms & Conditions</p><p class="text-muted" style="white-space: pre-line; line-height: 1.4;">${loadedData.company['Terms'] || 'Thank you for your business!'}</p></div><div class="col-5 d-flex justify-content-end"><div class="sign-line">Authorized Signatory</div></div></div>`;

  document.getElementById('invoicePreviewContent').innerHTML = invoiceHTML;
  document.getElementById('prevBtnDelete').onclick = () => { bootstrap.Modal.getInstance(document.getElementById('invoicePreviewModal')).hide(); confirmDelete('invoice', invId); };
  document.getElementById('prevBtnEdit').onclick = () => { bootstrap.Modal.getInstance(document.getElementById('invoicePreviewModal')).hide(); openEditInvoice(invId); };
  document.getElementById('prevBtnPrint').onclick = () => { printHTML(invoiceHTML); };
  new bootstrap.Modal(document.getElementById('invoicePreviewModal')).show();
  setTimeout(setInitialZoom, 200);
}
...
function showActionModal(type, id) {
  ...
  if (type === 'client') {
    ...
    html += `<button class="btn btn-light border w-100 text-start py-2 mb-2" onclick="closeActionModal(); openEditReceipt('${id}')"><i class="bi bi-pencil me-3 text-secondary"></i> Edit Receipt</button>`;
    html += `<button class="btn btn-light border w-100 text-start py-2 text-danger" onclick="closeActionModal(); confirmDelete('receipt', '${id}')"><i class="bi bi-trash me-3"></i> Delete Receipt</button>`;
  }
...
function renderTables() {
  document.getElementById('invoiceTableBody').innerHTML = loadedData.invoices.map(i => {
    let status = i.balance <= 0 ? '<span class="badge-paid">Paid</span>' : '<span class="badge-due">Due</span>';
    ...
    return `<tr onclick="openInvoicePreview('${i['Invoice ID']}')">
      ...
      <td class="d-none d-md-table-cell text-end text-nowrap">
        ${i.balance > 0 ? `<button class="btn btn-sm btn-light text-success me-1 mb-1" onclick="event.stopPropagation(); openAddReceipt('${i['Invoice ID']}')" title="Record Receipt"><i class="bi bi-cash-stack"></i></button>` : ''}
        <button class="btn btn-sm btn-light me-1 mb-1" onclick="event.stopPropagation(); printInvoice('${i['Invoice ID']}')" title="Print"><i class="bi bi-printer"></i></button>
        <button class="btn btn-sm btn-light me-1 mb-1" onclick="event.stopPropagation(); openEditInvoice('${i['Invoice ID']}')" title="Edit"><i class="bi bi-pencil"></i></button>
      </td>
      <td class="d-md-none p-0">
        <div class="p-3 pb-2">
            ...
            <div class="d-flex justify-content-between align-items-end">
              <div><div class="small fw-bold">Amt: ${netAmt}</div>${balText}</div>
              <div>${status}</div>
            </div>
        </div>
        ${i.balance > 0 ? `<div class="text-center py-2 border-top bg-light" onclick="event.stopPropagation(); openAddReceipt('${i['Invoice ID']}')"><span class="text-success fw-bold" style="font-size: 0.8rem;"><i class="bi bi-cash-stack me-2"></i>Record Receipt</span></div>` : ''}
      </td>
    </tr>`;
  }).join('') || '<tr><td colspan="7" class="text-center text-muted py-4">No invoices found.</td></tr>';

  document.getElementById('clientTableBody').innerHTML = loadedData.clients.map(c => {
    ...
    let contactHTML = '';
    if(c['Contact Name'] || c.Mobile) {
      let n = c['Contact Name'] ? `<i class="bi bi-person"></i> ${c['Contact Name']}` : ''; let m = c.Mobile ? `<i class="bi bi-telephone"></i> ${c.Mobile}` : ''; let sep = (c['Contact Name'] && c.Mobile) ? ' | ' : '';
      contactHTML = `<div class="text-muted small mb-1">${n}${sep}${m}</div>`;
    }
    return `<tr onclick="if(window.innerWidth < 768) showActionModal('client', '${c.ID}')">
      <td class="d-none d-md-table-cell fw-medium">${c.Name}</td>
      <td class="d-none d-md-table-cell text-muted">${c['Contact Name'] || '-'}</td>
      <td class="d-none d-md-table-cell text-muted">${c.Mobile || '-'}</td>
      <td class="d-none d-md-table-cell fw-bold ${c.balanceDue > 0 ? 'text-danger' : 'text-success'}">${(c.balanceDue||0).toFixed(2)}</td>
      <td class="d-none d-md-table-cell text-end text-nowrap">
        <button class="btn btn-sm btn-light text-success me-1 mb-1" onclick="event.stopPropagation(); openAddReceipt(null, '${c.ID}')" title="Record Receipt"><i class="bi bi-cash-stack"></i></button>
        <button class="btn btn-sm btn-info text-white me-1 mb-1" onclick="event.stopPropagation(); openStatement('${c.ID}')" title="Statement"><i class="bi bi-journal-text"></i></button>
        <button class="btn btn-sm btn-light mb-1" onclick="event.stopPropagation(); openEditClient('${c.ID}')" title="Edit"><i class="bi bi-pencil"></i></button>
      </td>
      <td class="d-md-none p-3"><div class="text-wrap fw-bold mb-1" style="font-size: 0.95rem;">${c.Name}</div>${contactHTML}<div class="small fw-bold mt-1">Balance: ${balText}</div></td>
    </tr>`;
  }).join('') || '<tr><td colspan="5" class="text-center text-muted py-4">No clients found.</td></tr>';

  document.getElementById('serviceTableBody').innerHTML = loadedData.services.map(s => 
  ...
  document.getElementById('receiptTableBody').innerHTML = loadedData.receipts.map(r => {
...
function openStatement(clientId) {
  ...
  generateStatement(); new bootstrap.Modal(document.getElementById('statementModal')).show();
}
...
function generateStatement() {
  ...
  loadedData.receipts.forEach(r => { let inv = loadedData.invoices.find(i => i['Invoice ID'] === r['Invoice ID']); let d = new Date(r.Date); let isForClient = (inv && inv['Client ID'] === currentStmtClient.ID) || (r['Client ID'] === currentStmtClient.ID); if(isForClient && d >= startDate && d <= endDate) { stmtData.push({ date: d, type: 'Receipt', ref: r['Receipt ID'], amount: -parseFloat(r.Amount) }); } });
  stmtData.sort((a, b) => a.date - b.date);
  let tbody = ''; let runTotal = 0;
  stmtData.forEach(row => { runTotal += row.amount; tbody += `<tr><td>${row.date.toLocaleDateString()}</td><td><span class="badge ${row.type==='Invoice'?'bg-primary':'bg-success'}">${row.type}</span></td><td>${row.ref}</td><td class="text-end ${row.amount < 0 ? 'text-success' : 'text-danger'}">${Math.abs(row.amount).toFixed(2)}</td></tr>`; });
  document.getElementById('statementTableBody').innerHTML = tbody || '<tr><td colspan="4" class="text-center text-muted">No records found.</td></tr>'; document.getElementById('stmtTotalDue').innerText = runTotal.toFixed(2);
}

// --- BULLETPROOF IFRAME PRINT ENGINE ---
function printHTML(content) {
    let oldFrame = document.getElementById('printIframe');
    ...
    doc.write(`
      ...
      <head>
        ...
        <style>
          @media print { @page { size: A4 portrait !important; margin: 10mm !important; } }
          body { padding: 0; font-family: sans-serif; color: #000; background: #fff; font-size: 12px; }
          .table-sm th, .table-sm td { padding: 4px 6px !important; }
          .sign-line { border-top: 1px solid #000; width: 200px; text-align: center; padding-top: 5px; font-weight: 600; margin-top: 40px; }
        </style>
      </head>
      <body>
        ${content}
...
function printInvoice(invId) {
    // If called directly from desktop list, generate HTML and print
    const inv = loadedData.invoices.find(i => i['Invoice ID'] === invId);
    const client = loadedData.clients.find(c => c.ID === inv['Client ID']);
    if(!inv) return;
    ...
    let itemsHtml = items.map(i => `<tr><td><strong class="text-dark">${i.Name}</strong><br><span class="text-muted">${i.Description}</span></td><td>${parseFloat(i.Rate).toFixed(2)}</td><td>${i.Quantity}</td><td class="text-end fw-bold">${parseFloat(i.Amount).toFixed(2)}</td></tr>`).join('');
    let contactHtml = client ? `${client['Contact Name'] || ''}<br>${client.Mobile || ''}<br>${client.Email || ''}` : '';
    let advanceRow = ''; if (bal < 0) { bal = 0; advanceRow = `<tr class="bg-light text-success"><td><h6 class="mt-2 fw-bold mb-2">Overpaid / Advance</h6></td><td><h6 class="mt-2 fw-bold mb-2">${Math.abs(inv.balance).toFixed(2)}</h6></td></tr>`; }
...
function printStatement() {
  let content = `
    ...
    <div class="row mb-4">
      <div class="col-6"><strong>Client:</strong> <span>${currentStmtClient.Name}</span></div>
      <div class="col-6 text-end"><strong>Date Generated:</strong> <span>${new Date().toLocaleDateString()}</span></div>
    </div>
    <table class="table table-bordered table-sm">
      <thead class="table-light"><tr><th>Date</th><th>Type</th><th>Ref #</th><th class="text-end">Amount</th></tr></thead>
      <tbody>${document.getElementById('statementTableBody').innerHTML}</tbody>
    </table>
    <h5 class="text-end fw-bold mt-3">Total Balance Due: <span class="text-danger">${document.getElementById('stmtTotalDue').innerText}</span></h5>
  `;
  printHTML(content);
}
