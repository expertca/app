  ...
  try {
    ...
    req.onsuccess = () => {
      ...
      apiCall(item.action, ...item.params)
        .then(() => { let dTx = db.transaction('syncQueue', 'readwrite'); dTx.objectStore('syncQueue').delete(item.id); dTx.oncomplete = () => processQueue(); })
...
function executeSave(action, ...params) {
  ...
  updateSyncStatus('Syncing...');
  apiCall(action, ...params).then(() => { loadApp(); }).catch(() => { saveToQueue(action, params); });
}
...
// --- UNIVERSAL PREVIEW & PDF SHARING LOGIC ---
let currentZoom = 1;
...
function setupPreviewPage(title, contentHtml, contextObj, onEdit, onDelete) {
    currentPreviewContext = contextObj;
    ...
    try {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
                title: `${currentPreviewContext.type} ${currentPreviewContext.id}`,
                text: `Attached is the ${currentPreviewContext.type} for ${currentPreviewContext.clientName}.`,
                files: [file]
            });
        } else {
...
function openInvoicePreview(invId) {
  ...
  setupPreviewPage('Invoice Preview', html, 
     { type: 'Invoice', id: inv['Invoice Number'], clientName: inv['Client Name'] }, 
     () => openEditInvoice(invId), 
     () => confirmDelete('invoice', invId)
  );
}

function openReceiptPreview(recId) {
    const rec = loadedData.receipts.find(r => r['Receipt ID'] === recId);
    const inv = loadedData.invoices.find(i => i['Invoice ID'] === rec['Invoice ID']);
    const cl = loadedData.clients.find(c => c.ID === rec['Client ID']);
    if(!rec) return;
    ...
    <table class="table table-bordered mt-4">
       <tbody>
          <tr><td class="w-25 text-muted fw-bold bg-light">Received From</td><td><strong class="fs-5">${clientName}</strong></td></tr>
          <tr><td class="text-muted fw-bold bg-light">Payment Mode</td><td>${rec['Payment Mode']}</td></tr>
          <tr><td class="text-muted fw-bold bg-light">Linked Invoice</td><td>${inv ? inv['Invoice Number'] : 'None (Advance)'}</td></tr>
       </tbody>
    </table>
    <div class="row print-footer mt-5 pt-5"><div class="col-7"></div><div class="col-5 d-flex justify-content-end"><div class="sign-line">Authorized Signatory</div></div></div>`;

    setupPreviewPage('Receipt Preview', html, 
       { type: 'Receipt', id: rec['Receipt ID'], clientName: clientName }, 
